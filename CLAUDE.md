...
| Key | Used for |
|-----|----------|
| `ANTHROPIC_API_KEY` | Claude Haiku — per-article "why it matters" generation |
...
`_debug.whySource` in the API response will be `"claude"` or `"fallback"`. If you see `"fallback"`, the Haiku call failed — check `ANTHROPIC_API_KEY` is set in Vercel and that `maxDuration` is high enough (currently 30s).
